import { httpResource } from './chunk-336PEC5Z.js';
    ...
    if (identityInParent === void 0) {
      ...
      return computed(() => {
        ...
        for (let i = 0; i < parentValue.length; i++) {
          ...
          if (
            isObject(data2) &&
            data2.hasOwnProperty(this.parent.structure.identitySymbol) &&
            data2[this.parent.structure.identitySymbol] === identityInParent
          ) {
...
function maxLength(path, maxLength2, config) {
  const MAX_LENGTH_MEMO = metadata(path, createMetadataKey(), (ctx) =>
    typeof maxLength2 === 'number' ? maxLength2 : maxLength2(ctx),
  );
...
function minLength(path, minLength2, config) {
  const MIN_LENGTH_MEMO = metadata(path, createMetadataKey(), (ctx) =>
    typeof minLength2 === 'number' ? minLength2 : minLength2(ctx),
  );
...
function pattern(path, pattern2, config) {
  const PATTERN_MEMO = metadata(path, createMetadataKey(), (ctx) =>
    pattern2 instanceof RegExp ? pattern2 : pattern2(ctx),
  );
...
function immediate() {}
var FORM_FIELD_PARSE_ERRORS = new InjectionToken(
...
function nativeControlCreate(host, parent, parseErrorsSource) {
  ...
  const parser = createParser(
    ...
    () => getNativeControlValue(input2, parent.state().value),
  );
